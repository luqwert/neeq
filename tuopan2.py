                ...
                for n in range(totalPages):
                    ...
                    for li in list:
                        ...
                        if result.fetchone():
                            ...
                            # label90.grid(row=9, column=0, columnspan=2, sticky=W)
                            time.sleep(3)
                time.sleep(10)  # 获取一个页面后休息10秒，防止请求服务器过快
            db.close()
                ...
                for n in range(totalPages):
                    ...
                    for li in list:
                        ...
                        if result.fetchone():
                            ...
                            # label90.grid(row=9, column=0, columnspan=2, sticky=W)
                            time.sleep(3)
                time.sleep(10)  # 获取一个页面后休息3秒，防止请求服务器过快
            db.close()
    logging.info("本次查询结束，10分钟后开始下次查询")
    time.sleep(600)
    s.root.after(50000, run(s,companyCd,companyName,startTime,endTime,receiveMail))
...
#     global sign
#     sign = 0
#     button51 = Button(s.root, text='停止', width=20, font=('幼圆', 12), fg='purple', command=lambda: start(s))
...
class _Main:  #调用SysTrayIcon的Demo窗口
    ...
    def main(s):
        ...
        global entry,entry11,entry21,entry31,entry41,sign
        sign = 0
        # 文本输入框前的提示文本
        ...
        button51 = Button(s.root, text='开始', width=20, font=('幼圆', 12), fg='purple', command=lambda:s.root.after(1000,run(s,entry.get().strip(),entry11.get().strip(),entry21.get().strip(),entry31.get().strip(),entry41.get().strip())))
        button51.grid(row=5, column=1)
